import socket
import os
from cryptography.hazmat.primitives import serialization, hashes
from cryptography.hazmat.primitives.asymmetric import rsa, padding
from cryptography.hazmat.primitives.asymmetric import x25519
from cryptography.hazmat.primitives.kdf.hkdf import HKDF
from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes

def load_keys():
    """Loads Alice's private identity key and Bob's public identity key."""
    with open("alice_private.pem", "rb") as f:
        alice_priv = serialization.load_pem_private_key(f.read(), password=None)
    with open("bob_public.pem", "rb") as f:
        bob_pub = serialization.load_pem_public_key(f.read())
    return alice_priv, bob_pub

def start_client():
    host, port = 'bob_node', 65432
    message = b"CLASSIFIED: The Eagle flies at midnight."
    alice_identity_priv, bob_identity_pub = load_keys()
    
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            s.connect((host, port))
            print(f"[*] Connected to Bob at {host}:{port}")
            
            # --- PHASE 1: AUTHENTICATED ECDH HANDSHAKE ---
            # 1. Generate Alice's ephemeral X25519PrivateKey
            alice_ephemeral_priv = x25519.X25519PrivateKey.generate()
            
            # 2. Extract Alice's X25519 public bytes
            alice_pub_bytes = alice_ephemeral_priv.public_key().public_bytes(
                encoding=serialization.Encoding.Raw,
                format=serialization.PublicFormat.Raw
            )

            # 3. Use alice_identity_priv to SIGN the X25519 public bytes using padding.PSS and hashes.SHA256()
            alice_signature = alice_identity_priv.sign(
                alice_pub_bytes,
                padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH),
                hashes.SHA256()
            )

            # 4. Send Alice's signature (256 bytes) and Alice's X25519 public key (32 bytes) to Bob.
            s.sendall(alice_signature + alice_pub_bytes)

            # 5. Receive Bob's data (Format: 256-byte signature + 32-byte X25519 public key)
            data = s.recv(288)
            bob_sig = data[:256]
            bob_pub_bytes = data[256:]

            # 6. Use bob_identity_pub to VERIFY Bob's signature against his X25519 public bytes.
            bob_identity_pub.verify(
                bob_sig,
                bob_pub_bytes,
                padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH),
                hashes.SHA256()
            )
            print("[+] Bob's signature verified.")

            # 7. Compute the shared_secret using the verified X25519 points.
            bob_ephemeral_pub = x25519.X25519PublicKey.from_public_bytes(bob_pub_bytes)
            shared_secret = alice_ephemeral_priv.exchange(bob_ephemeral_pub)

            # --- PHASE 2: KEY DERIVATION ---
            # Derive the AES key using HKDF (Same as Week 2)
            derived_key = HKDF(
                algorithm=hashes.SHA256(),
                length=32,
                salt=None,
                info=b'handshake data'
            ).derive(shared_secret)

            # --- PHASE 3: SECURE PAYLOAD ---
            # Encrypt and send the AES-GCM payload (Same as Week 1 & 2)
            # Extract nonce, tag, and ciphertext. 
            nonce  = os.urandom(12)

            cipher = Cipher(algorithms.AES(derived_key), modes.GCM(nonce))
            encryptor = cipher.encryptor()
            ciphertext = encryptor.update(message) + encryptor.finalize()
            tag = encryptor.tag

            s.sendall(nonce + tag + ciphertext)
            print("[*] Handshake complete. Encrypted payload sent.")
            
        except ConnectionRefusedError:
            print("[-] Connection refused. Is Bob's server running?")

if __name__ == "__main__":
    start_client()